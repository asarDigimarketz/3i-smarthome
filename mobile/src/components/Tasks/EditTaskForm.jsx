...
import { Image, ScrollView, Text, TextInput, TouchableOpacity, View, ActivityIndicator, Alert, ActionSheetIOS, Platform, TouchableWithoutFeedback } from "react-native";
...
const EditTaskForm = ({
  ...
  const [updating, setUpdating] = useState(false); // Add local updating state
  ...
  React.useEffect(() => {
    ...
    // Don't initialize if employees are not loaded yet
    ...
    if (editingTask.assignedTo) {
      if (Array.isArray(editingTask.assignedTo)) {
        // Handle array of populated objects from server
        assigneeIds = editingTask.assignedTo.map(assignee => {
      ...
      } else if (typeof editingTask.assignedTo === 'object') {
        // Single assignee object from server
        if (editingTask.assignedTo._id) {
          assigneeIds = [editingTask.assignedTo._id];
        } else if (editingTask.assignedTo.id) {
          assigneeIds = [editingTask.assignedTo.id];
        }
      } else if (typeof editingTask.assignedTo === 'string') {
        // Single assignee ID as string
        assigneeIds = [editingTask.assignedTo];
      }
    }

   

    // Only update if the assignee IDs are different and we have valid IDs
    if (assigneeIds.length > 0 && JSON.stringify(assigneeIds) !== JSON.stringify(editingTask.assignedTo)) {
      setEditingTask(prev => ({
        ...prev,
        assignedTo: assigneeIds
      }));
    } else if (assigneeIds.length === 0 && (!editingTask.assignedTo || editingTask.assignedTo.length === 0)) {
      setEditingTask(prev => ({
        ...prev,
        assignedTo: []
      }));
    }
  }, [editingTask.id, editingTask.assignedTo, employees]); // Run when task or employees change
  ...
  const getSelectedEmployeeNames = () => {
    ...
    if (!editingTask.assignedTo || editingTask.assignedTo.length === 0) {
 
      return 'Select Assignees';
    }
    
    const selectedNames = employees.filter(emp => editingTask.assignedTo.includes(emp.id)).map(emp => emp.name);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (editingTask.assignedTo && editingTask.assignedTo.length > 0) {
        formData.append('assignedTo', JSON.stringify(editingTask.assignedTo));
      }
      ...
      Alert.alert('Error', error.message || 'Failed to update task. Please check your connection and try again.');
    } finally {
      setUpdating(false);
    }
  };
    ...
    <View className="bg-white p-4 rounded-lg border border-gray-200 mb-4">
      ...
      <View className="mb-4">
        ...
        <View className="relative">
          <TouchableOpacity
            onPress={() => {
              console.log('🎯 EditTaskForm Assignee dropdown clicked, current state:', showAssigneeDropdown);
          ...
          >
            <View className="flex-row items-center flex-1">
              ...
              <Text className={`ml-2 ${editingTask.assignedTo && editingTask.assignedTo.length > 0 ? 'text-gray-900' : 'text-gray-500'}`}>
          ...
          {showAssigneeDropdown && (
            <TouchableWithoutFeedback onPress={() => setShowAssigneeDropdown(false)}>
              <View className="absolute top-12 left-0 bg-white rounded-lg shadow-xl z-50 w-full max-h-48 border border-gray-200" style={{ elevation: 10 }}>
                <ScrollView showsVerticalScrollIndicator={false} nestedScrollEnabled={true}>
                  {/* Unassigned option */}
                  <TouchableOpacity
                    className="px-4 py-3 border-b border-gray-100 flex-row items-center"
                    onPress={() => {
                      setEditingTask(prev => ({ ...prev, assignedTo: [] }));
                  ...
                  {employees.length === 0 ? (
                    ...
                    employees.map((employee) => (
                      ...
                      >
                        ...
                        {editingTask.assignedTo && editingTask.assignedTo.includes(employee.id) && (
