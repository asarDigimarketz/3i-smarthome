...
const EditEmployee = () => {
  ...
  useEffect(() => {
    if (!employeeDataString && employeeIdParam) {
      (async () => {
        try {
          const res = await auth.fetchWithAuth(`${API_CONFIG.API_URL}/api/employeeManagement/${employeeIdParam}`, {
            method: 'GET',
          });
                      if (!res.ok) {
            ...
            if (data && data.success && data.employee) {
              setFetchedEmployee(data.employee);
            setFormData((prev) => ({
              ...prev,
                ...data.employee,
                address: ensureAddress(data.employee.address),
            }));
          }
        } catch (e) {
