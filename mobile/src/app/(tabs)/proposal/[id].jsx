...
const ProposalDetail = () => {
  ...
  const fetchProposalDetails = async () => {
    try {
      setLoading(true);
      const response = await auth.fetchWithAuth(`${API_BASE_URL}/api/proposals/${id}`, {
        method: 'GET',
      });
  ...
  const updateProposalStatus = async (newStatus) => {
    try {
      ...
      const response = await auth.fetchWithAuth(`${API_BASE_URL}/api/proposals/${id}/field`, {
        method: 'PATCH',
        headers: {
