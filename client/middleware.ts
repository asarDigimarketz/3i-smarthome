...
export async function middleware(request: NextRequest) {
  try {
    ...
    if (pathname.startsWith("/api/")) {
      if (pathname === "/api/settings/general") {
        // Check if it's a client-side request
        const isClientSideRequest =
    ...
    if (token) {
      ...
      const isInactive = inactiveTime > 8 * 60 * 60 * 1000; // 2 hours
    ...
    // Check dashboard access (unified dashboard protection)
